...
import {
  OIWarmQueryCapability,
  type OIWarmQueryHooks,
} from './OIWarmQueryCapability.ts';
import {
  SampleMgmtCapability,
  type SampleMgmtHooks,
} from './SampleMgmtCapability.ts';
...
export async function getOIHooks(): Promise<OIWarmQueryHooks> {
  if (!oiHooks) {
    oiHooks = await CapabilityRunner(OIWarmQueryCapability()).Execute(
      undefined,
    );
  }
  return oiHooks;
...
export async function getWorkflowHooks(): Promise<SampleMgmtHooks> {
  if (!workflowHooks) {
    workflowHooks = await CapabilityRunner(SampleMgmtCapability()).Execute(
      undefined,
    );
  }
