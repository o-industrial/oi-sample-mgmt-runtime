...
export const ReviewRecordSchema = z.object({
  ...
  Type: z.enum(["reception", "reconciliation", "disposition"]).describe(
    "Review type",
  ),
  ...
  ValidationResult: z.enum(["passed", "failed", "warnings"]).describe(
    "Automated validation result",
  ),
  ExceptionFlags: z.array(z.string()).describe(
  ...
  Decision: z.enum(["approved", "rejected", "escalated"]).optional().describe(
    "Review decision",
  ),
