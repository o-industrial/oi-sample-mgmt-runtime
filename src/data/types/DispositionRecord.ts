...
export const DispositionRecordSchema = z.object({
  ...
  Decision: z.enum(['destroy', 'retain', 'deplete', 'pending']).describe(
  ...
  TreatmentStatus: z.enum(['blinded', 'unblinded']).optional().describe(
    'Blinding status',
  ),
  CustodianSignoff: CustodianSignoffSchema.optional().describe(
    'Custodian sign-off if completed',
  ),
  TwoPersonAuth: TwoPersonAuthSchema.optional().describe(
    'Two-person auth for controlled substances',
  ),
  EvidenceDocuments: z.array(EvidenceDocumentSchema).describe(
