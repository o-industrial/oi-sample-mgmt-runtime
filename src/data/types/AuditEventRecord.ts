...
export const AuditEventRecordSchema = z.object({
  ...
  ActionType: z.string().describe('Action performed (Scan, Create, Approve, Update, etc.)'),
  EntityType: z.string().describe('Type of entity affected (Sample, Manifest, Transfer, etc.)'),
  EntityId: z.string().describe('ID of the affected entity'),
  AlcoaPrinciple: z.string().describe('Primary ALCOA+ principle satisfied'),
  Status: z.enum(['success', 'failed']).describe('Whether the action succeeded'),
});
